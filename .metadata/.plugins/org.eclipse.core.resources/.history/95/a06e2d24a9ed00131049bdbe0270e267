...
import fileUtils.DropboxProxyFileSystem;
...
public class RmiRemoteDropboxProxyFileServerMain {
	...
	public static void main(String[] args) {
		...
		try {
			server = new RmiRemoteFileServer(new DropboxProxyFileSystem(
					service, accessToken));
		...
		IContactServer contact = RemoteFileServerUtils
				.getContactServer(contactAddress);
		...
		RemoteFileServerUtils.registerFileServer(serverName, user, contact,
				url);

		RemoteFileServerUtils.keepAlive(server, contact, serverName, user,
				url);
	}
}
